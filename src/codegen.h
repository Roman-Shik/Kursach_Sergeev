...
enum Instruction
{
	...
	LOAD,		// LOAD addr - загрузка слова данных в стек из памяти по 
				// адресу addr
	STORE,		// STORE addr - запись слова данных с вершины стека в память 
				// по адресу addr
	BLOAD,		// BLOAD addr - загрузка слова данных в стек из памяти 
				// по адресу addr + значение на вершине стека
	BSTORE,		// BSTORE addr - запись слова данных по адресу 
				// addr + значение на вершине стека
	PUSH,		// PUSH n - загрузка в стек константы n
	POP,		// удаление слова с вершины стека
	DUP,		// копирование слова на вершине стека
	ADD,		// сложение двух слов на вершине стека и запись результата 
				// вместо них
	SUB,		// вычитание двух слов на вершине стека и запись результата 
				// вместо них
	MULT,		// умножение двух слов на вершине стека и запись результата 
				// вместо них
	DIV,		// деление двух слов на вершине стека и запись результата 
				// вместо них
	INVERT,		// изменение знака слова на вершине стека
	COMPARE,	// COMPARE cmp - сравнение двух слов на вершине стека с 
				// помощью операции сравнения с кодом cmp
	JUMP,		// JUMP addr - безусловный переход по адресу addr
	JUMP_YES,	// JUMP_YES addr - переход по адресу addr, если на вершине 
				// стека значение 1
	JUMP_NO,	// JUMP_NO addr - переход по адресу addr, если на вершине 
				// стека значение 0
	INPUT,		// чтение целого числа со стандартного ввода и загрузка его 
				// в стек
...
public:
	...
	// Получение адреса, непосредственно следующего за последней 
	// инструкцией в программе
